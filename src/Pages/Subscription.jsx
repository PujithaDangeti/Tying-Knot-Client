...
const Subscription = () => {
  const { user, login } = useAuth(); // Use `user` and `login` from context
  const [activePlan, setActivePlan] = useState(null);
  const [successMessage, setSuccessMessage] = useState("");
  const navigate = useNavigate();
  ...
  const plans = [
    ...
    },
    {
      ...
      price: "₹2499",
      description:
        "Full access to search, messaging, and advanced matchmaking features.",
      benefits: [
        "Profile Viewing",
        "Unlimited Messaging",
        "Advanced Search",
    ...
    },
    {
      ...
      price: "₹4999",
      description:
        "Exclusive access to premium profiles, matchmaking, and VIP support.",
      benefits: [
        "Profile Viewing",
        "Unlimited Messaging",
        "Advanced Search",
  ...
  const handleSubscribe = async () => {
    ...
    const selectedPlan = plans.find((plan) => plan.id === activePlan);
    const updatedUser = { ...user, subscription: selectedPlan.name };

    try {
      await axios.put(`${API_URL}/${user.id}`, updatedUser); // Update user in database
      login(updatedUser); // Update local auth state

      setSuccessMessage(`You have subscribed to the ${selectedPlan.name} plan.`);
      setTimeout(() => navigate("/dashboard"), 2000);
    } catch (error) {
      console.error("Error updating subscription:", error);
      setSuccessMessage("Failed to update subscription. Please try again.");
    }
  };
